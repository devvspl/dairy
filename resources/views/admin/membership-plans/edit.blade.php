...
<div class="bg-white rounded-xl shadow-sm p-4 lg:p-6 border" style="border-color: var(--border);">
    ...
    <form method="POST" action="{{ route('admin.membership-plans.update', $plan) }}">
        ...
        <div class="space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $plan->name) }}" required class="w-full px-3 py-2 border rounded-lg" style="border-color: var(--border);">
                    @error('name')<p class="text-red-600 text-sm mt-1">{{ $message }}</p>@enderror
                </div>

                <div>
                    <label class="block text-sm font-medium mb-2" style="color: var(--text);">Slug (optional, auto-generated)</label>
                    <input type="text" name="slug" value="{{ old('slug', $plan->slug) }}" class="w-full px-3 py-2 border rounded-lg" style="border-color: var(--border);">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="number" step="0.01" name="price" value="{{ old('price', $plan->price) }}" required class="w-full px-3 py-2 border rounded-lg" style="border-color: var(--border);">
                ...
                <div>
                    ...
                    <input type="number" name="order" value="{{ old('order', $plan->order) }}" class="w-full px-3 py-2 border rounded-lg" style="border-color: var(--border);">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="badge" value="{{ old('badge', $plan->badge) }}" class="w-full px-3 py-2 border rounded-lg" style="border-color: var(--border);">
                ...
                <div>
                    ...
                    <input type="text" name="icon" value="{{ old('icon', $plan->icon) }}" placeholder="fa-fire" class="w-full px-3 py-2 border rounded-lg" style="border-color: var(--border);">
            ...
            <div class="flex items-center space-x-4">
                <label class="flex items-center">
                    <input type="checkbox" name="is_featured" value="1" {{ old('is_featured', $plan->is_featured) ? 'checked' : '' }} class="mr-2">
        ...
        <div class="flex space-x-3 mt-6">
            <button type="submit" class="px-4 py-2 rounded-lg text-white font-medium" style="background-color: var(--green);">Update Plan</button>
