...
@section('title', 'Create Membership Step')
@section('page-title', 'Create Membership Step')

@section('content')
<div class="bg-white rounded-xl shadow-sm p-4 lg:p-6 border" style="border-color: var(--border);">
    <div class="flex items-center justify-between mb-6">
        <h2 class="text-xl font-bold" style="color: var(--text);">Create New Step</h2>
    ...
    <form method="POST" action="{{ route('admin.membership-steps.store') }}">
        @csrf
        <div class="space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-medium mb-2" style="color: var(--text);">Step Number *</label>
                    <input type="number" name="step_number" value="{{ old('step_number', 1) }}" required class="w-full px-3 py-2 border rounded-lg" style="border-color: var(--border);">
        ...
        <div class="flex space-x-3 mt-6">
            <button type="submit" class="px-4 py-2 rounded-lg text-white font-medium" style="background-color: var(--green);">Create Step</button>
