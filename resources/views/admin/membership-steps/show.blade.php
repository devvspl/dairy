...
@section('title', 'View Membership Step')
@section('page-title', 'Membership Step Details')

@section('content')
<div class="bg-white rounded-xl shadow-sm p-4 lg:p-6 border" style="border-color: var(--border);">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
        <div class="flex items-center space-x-3">
            <span class="inline-flex items-center justify-center w-12 h-12 rounded-full text-lg font-bold text-white" style="background-color: var(--green);">{{ $step->step_number }}</span>
            <h2 class="text-xl font-bold" style="color: var(--text);">{{ $step->title }}</h2>
        </div>
        
        <div class="flex space-x-3 mt-4 sm:mt-0">
            <a href="{{ route('admin.membership-steps.edit', $step) }}" class="inline-flex items-center px-4 py-2 rounded-lg text-sm font-medium text-white hover:opacity-90 transition-opacity" style="background-color: var(--green);">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div class="p-4 rounded-lg border" style="border-color: var(--border);">
            <div class="flex items-center space-x-3 mb-3">
                ...
                <div>
                    <p class="text-sm font-medium" style="color: var(--muted);">Step Number</p>
                    <p class="text-base font-semibold" style="color: var(--text);">{{ $step->step_number }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="p-4 rounded-lg border" style="border-color: var(--border);">
            <div class="flex items-center space-x-3 mb-3">
                ...
                <div>
                    ...
                    <p class="text-base font-semibold" style="color: var(--text);">{{ $step->order }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="p-4 rounded-lg border" style="border-color: var(--border);">
            <div class="flex items-center space-x-3 mb-3">
                ...
                <div>
                    ...
                    <p class="text-base font-semibold" style="color: var(--text);">{{ $step->title }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="md:col-span-2 p-4 rounded-lg border" style="border-color: var(--border);">
            ...
            <p class="text-base" style="color: var(--text);">{{ $step->description }}</p>
        </div>
        @endif
    </div>
    ...
    <div class="mt-6 pt-6 border-t grid grid-cols-1 md:grid-cols-2 gap-4" style="border-color: var(--border);">
        <div class="p-4 rounded-lg" style="background-color: rgba(47, 74, 30, 0.05);">
            ...
            <p class="text-sm" style="color: var(--text);">{{ $step->created_at->format('M d, Y h:i A') }}</p>
        </div>
        <div class="p-4 rounded-lg" style="background-color: rgba(47, 74, 30, 0.05);">
            <p class="text-sm font-medium mb-1" style="color: var(--muted);">Last Updated</p>
            <p class="text-sm" style="color: var(--text);">{{ $step->updated_at->format('M d, Y h:i A') }}</p>
        </div>
    </div>

    <div class="mt-6 pt-6 border-t" style="border-color: var(--border);">
        <form method="POST" action="{{ route('admin.membership-steps.destroy', $step) }}" onsubmit="return confirm('Are you sure you want to delete this step?');">
            @csrf @method('DELETE')
            <button type="submit" class="px-6 py-3 bg-red-600 text-white rounded-lg font-semibold hover:bg-red-700 transition-colors">Delete Step</button>
        </form>
    </div>
</div>
@endsection
