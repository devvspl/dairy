...
<div class="bg-white rounded-xl shadow-sm p-4 lg:p-6 border" style="border-color: var(--border);">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
        <div>
            <h2 class="text-xl font-bold" style="color: var(--text);">{{ $category->title }}</h2>
        </div>
        
        <div class="flex space-x-3 mt-4 sm:mt-0">
            <a href="{{ route('admin.categories.edit', $category) }}" class="inline-flex items-center px-4 py-2 rounded-lg text-sm font-medium text-white hover:opacity-90 transition-opacity" style="background-color: var(--green);">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div class="p-4 rounded-lg border" style="border-color: var(--border);">
            <div class="flex items-center space-x-3 mb-3">
                ...
                <div>
                    ...
                    <p class="text-base font-semibold" style="color: var(--text);">{{ $category->title }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="p-4 rounded-lg border" style="border-color: var(--border);">
            <div class="flex items-center space-x-3 mb-3">
                ...
                <div>
                    <p class="text-sm font-medium" style="color: var(--muted);">Icon Type</p>
        ...
        <div class="p-4 rounded-lg border" style="border-color: var(--border);">
            <div class="flex items-center space-x-3 mb-3">
                ...
                <div>
                    ...
                    <p class="text-base font-semibold" style="color: var(--text);">{{ $category->order }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="p-4 rounded-lg border" style="border-color: var(--border);">
            <p class="text-sm font-medium mb-2" style="color: var(--muted);">Price Text</p>
            <p class="text-base" style="color: var(--text);">{{ $category->price_text }}</p>
        </div>
        @endif

        @if($category->bg_color)
        <div class="p-4 rounded-lg border" style="border-color: var(--border);">
            <p class="text-sm font-medium mb-2" style="color: var(--muted);">Background Color</p>
            <div class="flex items-center space-x-2">
                <div class="w-8 h-8 rounded border" style="background-color: {{ $category->bg_color }}; border-color: var(--border);"></div>
                <p class="text-sm font-mono" style="color: var(--text);">{{ $category->bg_color }}</p>
            </div>
        </div>
        @endif
        ...
        <div class="md:col-span-2 p-4 rounded-lg border" style="border-color: var(--border);">
            <p class="text-sm font-medium mb-2" style="color: var(--muted);">Link</p>
            <p class="text-sm" style="color: var(--text);">{{ $category->link }}</p>
        </div>
        @endif
        ...
        <div class="md:col-span-2 p-4 rounded-lg border" style="border-color: var(--border);">
            ...
            <pre class="text-sm font-mono bg-gray-50 p-3 rounded overflow-x-auto" style="color: var(--text);">{{ $category->svg_path }}</pre>
        </div>
        @endif
    </div>
    ...
    <div class="mt-6 pt-6 border-t" style="border-color: var(--border);">
        <form method="POST" action="{{ route('admin.categories.destroy', $category) }}" onsubmit="return confirm('Are you sure you want to delete this category?');">
            @csrf @method('DELETE')
            <button type="submit" class="px-6 py-3 bg-red-600 text-white rounded-lg font-semibold hover:bg-red-700 transition-colors">Delete Category</button>
        </form>
    </div>
</div>
@endsection
