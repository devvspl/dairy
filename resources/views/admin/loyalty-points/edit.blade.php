...
@section('title', 'Edit Loyalty Points')
@section('page-title', 'Edit Loyalty Points')

@section('content')
<div class="bg-white rounded-xl shadow-sm p-4 lg:p-6 border" style="border-color: var(--border);">
    <div class="flex items-center justify-between mb-6">
        <h2 class="text-xl font-bold" style="color: var(--text);">Edit Loyalty Points</h2>
    ...
    <form method="POST" action="{{ route('admin.loyalty-points.update', $loyaltyPoint) }}">
        @csrf
        @method('PUT')
                ...
                <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="number" name="points" value="{{ old('points', $loyaltyPoint->points) }}" required min="1" class="w-full px-3 py-2 border rounded-lg" style="border-color: var(--border);">
                        ...
                        <div>
                            ...
                            <select name="type" required class="w-full px-3 py-2 border rounded-lg" style="border-color: var(--border);">
                                ...
                                <option value="redeemed" {{ old('type', $loyaltyPoint->type) == 'redeemed' ? 'selected' : '' }}>Redeemed</option>
                                <option value="expired" {{ old('type', $loyaltyPoint->type) == 'expired' ? 'selected' : '' }}>Expired</option>
                                <option value="adjusted" {{ old('type', $loyaltyPoint->type) == 'adjusted' ? 'selected' : '' }}>Adjusted</option>
                            </select>
                    ...
                    <div>
                        ...
                        <input type="text" name="reason" value="{{ old('reason', $loyaltyPoint->reason) }}" required class="w-full px-3 py-2 border rounded-lg" style="border-color: var(--border);">
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="3" class="w-full px-3 py-2 border rounded-lg" style="border-color: var(--border);">{{ old('description', $loyaltyPoint->description) }}</textarea>
                        @error('description')<p class="text-red-600 text-sm mt-1">{{ $message }}</p>@enderror
                    </div>

                    <div>
                        <label class="block text-sm font-medium mb-2" style="color: var(--text);">Expiry Date</label>
                        <input type="date" name="expires_at" value="{{ old('expires_at', $loyaltyPoint->expires_at?->format('Y-m-d')) }}" class="w-full px-3 py-2 border rounded-lg" style="border-color: var(--border);">
