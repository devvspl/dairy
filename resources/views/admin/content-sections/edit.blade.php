...
<div class="bg-white rounded-xl shadow-sm p-4 lg:p-6 border" style="border-color: var(--border);">
    <div class="flex items-center justify-between mb-6">
        <h2 class="text-xl font-bold" style="color: var(--text);">Edit Content Section</h2>
    ...
    <form method="POST" action="{{ route('admin.content-sections.update', $contentSection) }}">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <input type="text" name="section_key" value="{{ old('section_key', $contentSection->section_key) }}" required class="w-full px-3 py-2 border rounded-lg font-mono" style="border-color: var(--border);">
                ...
                <div>
                    ...
                    <input type="text" name="kicker" value="{{ old('kicker', $contentSection->kicker) }}" class="w-full px-3 py-2 border rounded-lg" style="border-color: var(--border);">
                ...
                <div>
                    ...
                    <input type="text" name="title" value="{{ old('title', $contentSection->title) }}" class="w-full px-3 py-2 border rounded-lg" style="border-color: var(--border);">
                    @error('title')<p class="text-red-600 text-sm mt-1">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="block text-sm font-medium mb-2" style="color: var(--text);">Description</label>
                    <textarea name="description" rows="3" class="w-full px-3 py-2 border rounded-lg" style="border-color: var(--border);">{{ old('description', $contentSection->description) }}</textarea>
                ...
                <div>
                    ...
                    <textarea name="points" rows="3" class="w-full px-3 py-2 border rounded-lg font-mono text-sm" style="border-color: var(--border);">{{ old('points', $contentSection->points ? json_encode($contentSection->points) : '') }}</textarea>
                ...
                <div>
                    ...
                    <textarea name="buttons" rows="3" class="w-full px-3 py-2 border rounded-lg font-mono text-sm" style="border-color: var(--border);">{{ old('buttons', $contentSection->buttons ? json_encode($contentSection->buttons) : '') }}</textarea>
                ...
                <div>
                    ...
                    <textarea name="meta" rows="2" class="w-full px-3 py-2 border rounded-lg font-mono text-sm" style="border-color: var(--border);">{{ old('meta', $contentSection->meta ? json_encode($contentSection->meta) : '') }}</textarea>
                    @error('meta')<p class="text-red-600 text-sm mt-1">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="flex items-center">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $contentSection->is_active) ? 'checked' : '' }} class="mr-2">
                        <span class="text-sm" style="color: var(--text);">Active</span>
                    </label>
                </div>
            </div>
            <div class="flex space-x-3 mt-6">
                <button type="submit" class="px-4 py-2 rounded-lg text-white font-medium" style="background-color: var(--green);">Update Section</button>
