...
@section('title', 'View Membership Benefit')
@section('page-title', 'Membership Benefit Details')

@section('content')
<div class="bg-white rounded-xl shadow-sm p-4 lg:p-6 border" style="border-color: var(--border);">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
        <div>
            <h2 class="text-xl font-bold" style="color: var(--text);">{{ $benefit->title }}</h2>
        </div>
        
        <div class="flex space-x-3 mt-4 sm:mt-0">
            <a href="{{ route('admin.membership-benefits.edit', $benefit) }}" class="inline-flex items-center px-4 py-2 rounded-lg text-sm font-medium text-white hover:opacity-90 transition-opacity" style="background-color: var(--green);">
                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                Edit
            </a>
            <a href="{{ route('admin.membership-benefits.index') }}" class="inline-flex items-center px-4 py-2 rounded-lg text-sm font-medium hover:bg-gray-100 transition-colors" style="color: var(--text); border: 1px solid var(--border);">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div class="p-4 rounded-lg border" style="border-color: var(--border);">
            <div class="flex items-center space-x-3 mb-3">
                ...
                <div>
                    ...
                    <p class="text-base font-semibold" style="color: var(--text);">{{ $benefit->title }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="p-4 rounded-lg border" style="border-color: var(--border);">
            <div class="flex items-center space-x-3 mb-3">
                ...
                <div>
                    ...
                    <p class="text-base font-semibold" style="color: var(--text);">{{ $benefit->order }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="p-4 rounded-lg border" style="border-color: var(--border);">
            ...
            <div class="flex items-center space-x-2">
                <i class="fas {{ $benefit->icon }} text-2xl" style="color: var(--green);"></i>
                <p class="text-sm font-mono" style="color: var(--text);">{{ $benefit->icon }}</p>
            </div>
        </div>
        @endif
        ...
        <div class="md:col-span-2 p-4 rounded-lg border" style="border-color: var(--border);">
            ...
            <p class="text-base" style="color: var(--text);">{{ $benefit->description }}</p>
        </div>
        @endif
    </div>
    ...
    <div class="mt-6 pt-6 border-t grid grid-cols-1 md:grid-cols-2 gap-4" style="border-color: var(--border);">
        <div class="p-4 rounded-lg" style="background-color: rgba(47, 74, 30, 0.05);">
            ...
            <p class="text-sm" style="color: var(--text);">{{ $benefit->created_at->format('M d, Y h:i A') }}</p>
        </div>
        <div class="p-4 rounded-lg" style="background-color: rgba(47, 74, 30, 0.05);">
            <p class="text-sm font-medium mb-1" style="color: var(--muted);">Last Updated</p>
            <p class="text-sm" style="color: var(--text);">{{ $benefit->updated_at->format('M d, Y h:i A') }}</p>
        </div>
    </div>

    <div class="mt-6 pt-6 border-t" style="border-color: var(--border);">
        <form method="POST" action="{{ route('admin.membership-benefits.destroy', $benefit) }}" onsubmit="return confirm('Are you sure you want to delete this benefit?');">
            @csrf @method('DELETE')
            <button type="submit" class="px-6 py-3 bg-red-600 text-white rounded-lg font-semibold hover:bg-red-700 transition-colors">Delete Benefit</button>
        </form>
    </div>
</div>
@endsection
