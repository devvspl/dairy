...
<div class="space-y-4 lg:space-y-6">
    <div class="bg-white rounded-xl shadow-sm p-4 lg:p-6 border" style="border-color: var(--border);">
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-4 lg:space-y-0">
            <div>
                <h2 class="text-xl font-bold" style="color: var(--text);">All Testimonials</h2>
                <p class="text-sm mt-1" style="color: var(--muted);">Manage customer testimonials</p>
            </div>
            <div class="flex flex-col sm:flex-row space-y-3 sm:space-y-0 sm:space-x-3">
                ...
                <a href="{{ route('admin.testimonials.create') }}" class="inline-flex items-center justify-center px-4 py-2 rounded-lg text-white text-sm font-medium" style="background-color: var(--green);">
            ...
            <tbody class="divide-y" style="border-color: var(--border);">
                @forelse($testimonials as $testimonial)
                    <tr class="hover:bg-gray-50">
                        <td class="px-6 py-4">
                            <p class="font-medium" style="color: var(--text);">{{ $testimonial->name }}</p>
                            @if($testimonial->location)<p class="text-sm" style="color: var(--muted);">{{ $testimonial->location }}</p>@endif
                        </td>
                        <td class="px-6 py-4 hidden md:table-cell"><span class="text-sm" style="color: var(--text);">{{ $testimonial->order }}</span></td>
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-end space-x-2">
                                <a href="{{ route('admin.testimonials.show', $testimonial) }}" class="p-2 rounded-lg hover:bg-gray-100" style="color: var(--green);" title="View">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                                </a>
                                <a href="{{ route('admin.testimonials.edit', $testimonial) }}" class="p-2 rounded-lg hover:bg-gray-100" style="color: var(--green);">
                ...
                @empty
                    <tr><td colspan="4" class="px-6 py-12 text-center"><p class="text-lg font-medium" style="color: var(--text);">No testimonials found</p></td></tr>
                @endforelse
            </tbody>
        </table>
        @if($testimonials->hasPages())<div class="px-6 py-4 border-t" style="border-color: var(--border);">{{ $testimonials->links() }}</div>@endif
    </div>
</div>
@endsection
