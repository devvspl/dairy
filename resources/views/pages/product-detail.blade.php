...
@section('content')
    ...
    <style>
        ...
        .apw-pd-thumb:hover {
            ...
            box-shadow: 0 10px 20px rgba(20, 25, 18, .08)
        }

        .apw-pd-thumb.is-active {
            border-color: rgba(41, 62, 138, .45);
            box-shadow: 0 14px 28px rgba(41, 62, 138, .12)
        }
        ...
        .apw-pd-bgBrand {
            background: rgba(41, 62, 138, .10);
            border: 1px solid rgba(41, 62, 138, .18)
        }

        .apw-pd-bgGreen {
            background: rgba(47, 74, 30, .08);
            border: 1px solid rgba(47, 74, 30, .16)
        }
        ...
        .apw-pd-chip:hover {
            ...
            box-shadow: 0 10px 18px rgba(20, 25, 18, .07)
        }

        .apw-pd-chip.is-active {
            border-color: rgba(41, 62, 138, .45);
            box-shadow: 0 14px 26px rgba(41, 62, 138, .12)
        }
        ...
        .apw-pd-btn.primary {
            background: #293e8a;
            color: #fff;
            box-shadow: 0 16px 35px rgba(41, 62, 138, .22)
        }

        .apw-pd-btn.primary:hover {
            transform: translateY(-2px)
        }

        .apw-pd-btn.ghost {
            background: #fff;
            border-color: rgba(31, 42, 26, .10);
            color: #1f2a1a
        }
        ...
        .apw-pd-mini:hover {
            transform: translateY(-1px);
            box-shadow: 0 14px 26px rgba(20, 25, 18, .08)
        }
        ...
        .apw-pd-tabs {
            ...
            gap: 10px;
            flex-wrap: wrap;
            border-bottom: 1px solid rgba(31, 42, 26, .10);
            padding-bottom: 12px;
            margin-bottom: 12px
        }
        ...
        .apw-pd-tab.is-active {
            background: rgba(41, 62, 138, .08);
            border-color: rgba(41, 62, 138, .22);
            color: #293e8a
        }
        ...
        .apw-pd-spec tr:last-child th,
        .apw-pd-spec tr:last-child td {
            border-bottom: 0
        }
        ...
        .apw-pd-relCard:hover {
            transform: translateY(-3px);
            box-shadow: 0 18px 36px rgba(20, 25, 18, .10)
        }
        ...
        <div class="apw-pd-container">
                ...
                <div class="apw-pd-banner__in">
                    <div class="apw-pd-banner__copy">
                        ...
                        <div class="apw-pd-banner__cta">
                            <a class="apw-pd-btn primary" href="#apwPdRelated">Shop Now</a>
                            <a class="apw-pd-btn ghost" href="{{ route('contact') }}">Contact Us</a>
                        </div>
                    </div>
                ...
                <div class="apw-pd-card apw-pd-boxbg apw-pd-gallery">
                    <div class="apw-pd-gallery__in">
                        <div class="apw-pd-mainshot">
                            <img id="apwPdMainImage" src="{{ asset($product->images[0] ?? $product->image) }}"
                                alt="{{ $product->name }}">
                        </div>
                        ...
                        <div class="apw-pd-thumbs" role="list">
                            @if ($product->images && count($product->images) > 0)
                                @foreach ($product->images as $index => $img)
                                    <button class="apw-pd-thumb {{ $index == 0 ? 'is-active' : '' }}" type="button"
                                        data-img="{{ asset($img) }}" aria-label="Thumbnail {{ $index + 1 }}">
                                        <img src="{{ asset($img) }}" alt="Thumbnail {{ $index + 1 }}">
                                    </button>
                                @endforeach
                            @else
                                <button class="apw-pd-thumb is-active" type="button"
                                    data-img="{{ asset($product->image) }}" aria-label="Thumbnail 1">
                ...
                <div class="apw-pd-card apw-pd-pad apw-pd-boxbg">
                        ...
                        <div class="apw-pd-badges">
                            @foreach (array_slice($product->features, 0, 3) as $feature)
                                @if (isset($feature['title']) && $feature['title'])
                                    <span class="apw-pd-badge"><span class="apw-pd-dot"></span>
                                        {{ $feature['title'] }}</span>
                                @endif
                            @endforeach
                        </div>
                    ...
                    <div class="apw-pd-priceBox apw-pd-boxbg">
                        ...
                        @if ($product->storage_temp || $product->shelf_life || $product->best_for)
                            <div class="apw-pd-quick">
                                @if ($product->storage_temp)
                                    <div class="apw-pd-q apw-pd-bgGreen">
                                        <b>Storage</b><span>{{ $product->storage_temp }}</span>
                                    </div>
                                @endif
                                @if ($product->shelf_life)
                                    <div class="apw-pd-q apw-pd-bgBrand"><b>Shelf
                                            Life</b><span>{{ $product->shelf_life }}</span></div>
                                @endif
                                @if ($product->best_for)
                                    <div class="apw-pd-q apw-pd-bgGreen"><b>Best
                                            For</b><span>{{ $product->best_for }}</span></div>
                                @endif
                            </div>
                        @endif
                        ...
                        <div class="apw-pd-miniActions">
                            ...
                            <!-- WhatsApp Button -->
                            <button class="apw-pd-mini apw-pd-boxbg" type="button"
                                onclick="window.open('https://wa.me/919999787973', '_blank')">
                                <span class="apw-pd-ic">💬</span> WhatsApp Order
                            </button>

                            <!-- Call Button -->
                            <button class="apw-pd-mini apw-pd-boxbg" type="button"
                                onclick="window.location.href='tel:+919999787973'">
                                <span class="apw-pd-ic">📞</span> Call for Bulk
                            </button>
            ...
            <section class="apw-pd-details">
                    ...
                    <div class="apw-pd-tabs" role="tablist">
                        ...
                        @if (
                            $product->storage_instructions &&
                                is_array($product->storage_instructions) &&
                                count($product->storage_instructions) > 0)
                ...
                <aside class="apw-pd-card apw-pd-boxbg">
                    <div class="apw-pd-sideHead">
                        <b>Delivery & Policy</b>
                        <span style="color:#5c6b55;font-size:13px">{{ ucfirst($product->category ?? '') }}</span>
                    </div>
                    <div class="apw-pd-sideBody">
                        ...
                        <div style="margin-top:12px;display:grid;gap:10px">
                            <a class="apw-pd-btn ghost" href="{{ route('terms-conditions') }}" style="width:100%">View
                                Terms</a>
                            <a class="apw-pd-btn ghost" href="{{ route('contact') }}" style="width:100%">Contact Us</a>
                        </div>
                    </div>
                </aside>
            </section>
            ...
            <section class="apw-pd-related" id="apwPdRelated">
                <div class="apw-pd-relHead">
                    ...
                    <a class="apw-pd-btn ghost" href="{{ route('products') }}"
                        style="padding:10px 12px;border-radius:999px">View All</a>
                </div>
                ...
                <div class="apw-pd-relGrid">
                    @forelse($relatedProducts as $relProduct)
                        <a class="apw-pd-relCard apw-pd-boxbg" href="{{ route('product.detail', $relProduct->slug) }}">
                            <div class="apw-pd-relImg"><img src="{{ asset($relProduct->main_image) }}"
                                    alt="{{ $relProduct->name }}"></div>
