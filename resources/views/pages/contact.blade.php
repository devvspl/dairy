...
@section('content')
    ...
    <style>
        ...
        #cpgContactPage .cpg-sec-head-left {
            text-align: left;
            margin-left: 0;
        }
        ...
        #cpgContactPage .cpg-faq-item.cpg-open .cpg-faq-a {
            display: block;
        }
    ...
    <main id="cpgContactPage">
        ...
        <section class="cpg-hero">
            <div class="cpg-hero-bg" @if($contactPage && $contactPage->hero_image) style="background-image: url('{{ asset($contactPage->hero_image) }}');" @endif></div>
            ...
            <div class="container cpg-hero-inner">
                <div class="cpg-hero-content">
                    ...
                    <h1>{{ $contactPage->hero_title ?? 'Let\'s talk—quick support, clear answers.' }}</h1>
                    <p>{{ $contactPage->hero_description ?? 'Have a question about products, Share your details and we\'ll connect shortly.' }}</p>

                    <div class="cpg-hero-actions">
                        @if($contactPage && $contactPage->hero_phone)
                        <a class="cpg-btn cpg-btn-primary" href="tel:{{ str_replace(' ', '', $contactPage->hero_phone) }}">
                            <i class="fa-solid fa-phone"></i> Call Now
                        </a>
                        @endif
                        @if($contactPage && $contactPage->hero_email)
                        <a class="cpg-btn cpg-btn-outline" href="mailto:{{ $contactPage->hero_email }}">
            ...
            <div class="container">
                ...
                <div class="cpg-info-grid">
                    ...
                    <div class="cpg-info-card">
                        ...
                        <h3>{{ $contactPage->phone_title ?? 'Call Us' }}</h3>
                        <p>{{ $contactPage->phone_description ?? 'Talk to our support team for quick help and guidance.' }}</p>
                        <a class="cpg-link" href="tel:{{ str_replace(' ', '', $contactPage->phone_number ?? '') }}"><i class="fa-solid fa-arrow-right"></i> {{ $contactPage->phone_number ?? '+91 12345 67890' }}</a>
                    </div>

                    <div class="cpg-info-card">
                        <div class="cpg-info-ico"><i class="fa-solid fa-envelope"></i></div>
                        <h3>{{ $contactPage->email_title ?? 'Email' }}</h3>
                        <p>{{ $contactPage->email_description ?? 'Send your query and we\'ll reply with clear details.' }}</p>
                        <a class="cpg-link" href="mailto:{{ $contactPage->email_address ?? 'hello@example.com' }}"><i class="fa-solid fa-arrow-right"></i> {{ $contactPage->email_address ?? 'hello@example.com' }}</a>
                    </div>

                    <div class="cpg-info-card">
                        <div class="cpg-info-ico"><i class="fa-solid fa-location-dot"></i></div>
                        <h3>{{ $contactPage->address_title ?? 'Visit' }}</h3>
                        <p>{{ $contactPage->address_description ?? 'Office / Store address line will come here for users.' }}</p>
                    ...
                    <div class="cpg-card cpg-map" id="cpgMap">
                        <div class="cpg-map-top">
                            <div>
                                ...
                                <h3 class="pt-2">{{ $contactPage->map_title ?? 'Find us on map' }}</h3>
                            </div>
                            @if($contactPage->map_link)
                            <a class="cpg-btn cpg-btn-outline" href="{{ $contactPage->map_link }}" target="_blank" rel="noopener">
                                <i class="fa-solid fa-location-arrow"></i> Open in Maps
                            </a>
                            @endif
                        </div>

                        @if($contactPage->map_embed_url)
        ...
        <!-- OPTIONAL FAQ -->
        @if($contactPage && $contactPage->faqs && count($contactPage->faqs) > 0)
        <section class="cpg-faq" id="cpgFaq">
            <div class="container">
                ...
                <div class="cpg-faq-grid">
                    @php
                        $half = ceil(count($contactPage->faqs) / 2);
                        $leftFaqs = array_slice($contactPage->faqs, 0, $half);
                        $rightFaqs = array_slice($contactPage->faqs, $half);
                    @endphp
                    ...
                    <div class="cpg-faq-col">
                        ...
                        <div class="cpg-faq-item">
                            ...
                            <div class="cpg-faq-a">{{ $faq['answer'] }}</div>
                        </div>
                        @endforeach
                    </div>
